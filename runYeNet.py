...
try:
	from numpy import array, load as npLoad, sum as npSum
...
EXIT_FAILURE = 1
EOF = (-1)
trainingCoverFolderPath = "dataSet/trainingCover"
trainingStegoFolderPath = "dataSet/trainingStego"
validationCoverFolderPath = "dataSet/validationCover"
validationStegoFolderPath = "dataSet/validationStego"
testingCoverFolderPath = "dataSet/testingCover"
testingStegoFolderPath = "dataSet/testingStego"
maxEpoch = 10
...
class SRMConv(Module):
	def __init__(self, preProcessingTuple:tuple, filterFilePath:str) -> None:
		super().__init__()
		self.device = torchDevice("cuda:0" if is_available() else "cpu")
		self.srm = from_numpy(npLoad(filterFilePath)).to(self.device, dtype = torchFloat)
...
class YeNet(Module):
	...
	def forward(self, image:tensor) -> tensor:
		out = SRMConv(self.preProcessingTuple, self.filterFilePath)(image)
		out = Sequential(		\
			self.layer1, 	\
			self.layer2, 	\
			self.layer3, 	\
...
class RunYeNet:
	...
	def __init__(							\
		...
		preProcessingTuple:tuple, filterFilePath:str, checkpointFolderPath:str, 	\
	...
	def load(self:object) -> None:
		...
		try:
			self.model = YeNet(self.preProcessingTuple, self.filterFilePath)
		except Exception as e:
			print("Failed loading the filter \"{0}\". Details are as follows. \n{1}".format(filterFilePath, e))
			return False
		...
		if RunYeNet.handleFolder(self.resultFolderPath):
			...
			if logFigure:
				...
				bRet = self.draw(										\
					...
					color = "orange", marker = "x", legend = ["Accuracy"], xlabel = "Batch", ylabel = "Accuracy", 		\
				...
				bRet = self.draw(									\
					...
					color = "orange", marker = "x", legend = ["Loss"], xlabel = "Batch", ylabel = "Loss", 		\
				...
				bRet = self.draw(									\
					...
					color = "orange", marker = "x", legend = ["Time"], xlabel = "Batch", ylabel = "Time (s)", 	\
...
def main() -> int:
	try:
		runYeNet = RunYeNet(					\
			...
			initialLearningRate, preProcessingTuple, filterFilePath, 	\
